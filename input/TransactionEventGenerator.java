...
public class TransactionEventGenerator implements EventQueue {
    ...
    protected int drawToAddress(int from) {
        int nodesInGroup = SimScenario.getInstance().getNrofNodeInGroups();
        int to;
        
        if (from >= 1 && from <=1* nodesInGroup) {
            to = 8*nodesInGroup+1;
        } else if (from <= 2*nodesInGroup) {
            to = 8*nodesInGroup+2;
        } else if (from <= 3*nodesInGroup) {
            to = 8*nodesInGroup+3;
        } else if (from <= 4*nodesInGroup) {
            to = 8*nodesInGroup+4;
        } else if (from <= 5*nodesInGroup) {
            to = 8*nodesInGroup+5;
        } else if (from <= 6*nodesInGroup) {
            to = 8*nodesInGroup+6;
        } else if (from <= 7*nodesInGroup) {
            to = 8*nodesInGroup+7;
        } else {
            to = 8*nodesInGroup+8;
        }
    ...
    public ExternalEvent nextEvent() {
        ...
        if (!senderHost.toString().startsWith("node") || senderHost.getWallet() == null) {
            return null;
        }
